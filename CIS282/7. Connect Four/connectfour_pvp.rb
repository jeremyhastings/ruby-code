...
def acquire_player_move(player_moving)

  puts
  print "Select a column to place your piece ( #{player_moving} ): "
...
while game_over == 0
  ...
  x_player_selection = acquire_player_move("X")
  if x_player_selection >= 1 && x_player_selection <= 8
    x_row_move = process_selection(simple_board_array, x_player_selection - 1)
    if x_row_move != -1
      simple_board_array[x_row_move][x_player_selection - 1] = "X"
      print_board(simple_board_array)
      game_over = check_for_win(simple_board_array, x_row_move, x_player_selection - 1, "X")
    end
    if game_over == 0 && x_row_move != -1
      y_player_selection = acquire_player_move("0")
      if y_player_selection >= 1 && y_player_selection <= 8
        y_row_move = process_selection(simple_board_array, y_player_selection - 1)
        if y_row_move != -1
          simple_board_array[y_row_move][y_player_selection - 1] = "0"
          print_board(simple_board_array)
          game_over = check_for_win(simple_board_array, y_row_move, y_player_selection - 1, "0")
        end
      end
    end
